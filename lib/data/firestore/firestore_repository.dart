...
class FirestoreRepository {
  ...
  FirestoreRepository({FirebaseFirestore? firestore}) : _firestore = firestore;
  ...
  Stream<List<Place>> watchPlaces() {
    try {
      return _safeFirestore.collection('places').snapshots().map((snapshot) {
  ...
  Stream<List<Contributor>> watchContributors() {
    try {
      return _safeFirestore.collection('Council').snapshots().map((snapshot) {
        final list = snapshot.docs
    ...
    try {
      return _safeFirestore
          ...
          .map((snapshot) {
            final list = snapshot.docs
                .map(
                  (doc) => Contributor.fromJson({...doc.data(), 'id': doc.id}),
                )
                .toList();
            // Sort by order field
            list.sort((a, b) => (a.order ?? 999).compareTo(b.order ?? 999));
  ...
  Stream<List<Project>> watchProjects() {
    try {
      return _safeFirestore.collection('projects').snapshots().map((snapshot) {
        final list = snapshot.docs
  ...
  Stream<List<AppEvent>> watchEvents() {
    try {
      return _safeFirestore.collection('events').snapshots().map((snapshot) {
        final list = snapshot.docs
  ...
  Stream<List<UniversityModel>> watchUniversities() {
    try {
      return _safeFirestore.collection('universities').snapshots().map((
        snapshot,
      ) {
        final list = snapshot.docs
  ...
  Stream<List<OversightCommittee>> watchOversightCommittees() {
    try {
      return _safeFirestore.collection('OversightCommittee').snapshots().map((
        snapshot,
      ) {
        final list = snapshot.docs
            .map(
              (doc) =>
                  OversightCommittee.fromJson({...doc.data(), 'id': doc.id}),
            )
            .toList();
        // Sort by order field
        list.sort((a, b) => (a.order ?? 999).compareTo(b.order ?? 999));
    ...
    try {
      return _safeFirestore
          ...
          .map((snapshot) {
            final list = snapshot.docs
                .map(
                  (doc) => OversightCommittee.fromJson({
                    ...doc.data(),
                    'id': doc.id,
                  }),
                )
  ...
  Future<List<int>> getOversightCommitteeYears() async {
    try {
      final snapshot = await _safeFirestore
          .collection('OversightCommittee')
          .get();
      final years = <int>{};
      for (var doc in snapshot.docs) {
