...
class ContributorsTimelineScreen extends ConsumerStatefulWidget {
  const ContributorsTimelineScreen({super.key});

  @override
  ConsumerState<ContributorsTimelineScreen> createState() =>
      _ContributorsTimelineScreenState();
}

class _ContributorsTimelineScreenState
    extends ConsumerState<ContributorsTimelineScreen> {
  int? _selectedYear;

  @override
  Widget build(BuildContext context) {
    final yearsAsync = ref.watch(contributorYearsProvider);

    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context).pastContributors),
      ),
...
class _ContributorsList extends ConsumerWidget {
  const _ContributorsList({required this.year});

  final int year;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final contributorsAsync = ref.watch(contributorsByYearProvider(year));
      ...
      data: (contributors) {
        ...
        return GridView.builder(
          ...
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
            crossAxisCount: 2,
            crossAxisSpacing: 20,
            mainAxisSpacing: 30, // More vertical space like the picture
            childAspectRatio: 0.75, // Taller aspect ratio for the photo + text
          ),
            ...
            return InkWell(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    width: 100, // Bigger size matching reference
                    height: 100,
